...
export const ProfileNotes = ({ userProfile, token, setProfile }) => {
  ...
  const handleNotesSubmit = (value, text)=>{
    ...
    fetch(`/updatedetails/${text}`, {
      ...
      .then((data) => {
        ...
        console.log("updatedetails/notes result is ", data)
  ...
  return (
    <>
      <div className="font-poppins text-center  text-lg mb-3 text-skin-base underline">About Me:</div>
      {userProfile.user.notes ? (
        <>
          ...
          <div className="text-xl">{userProfile.user.notes}{token && <i className="material-icons modal-trigger ml-2 mr-1 mb-0 bg-skin-fill border-skin-base border-solid border-2 select-none"
          ...
          <div id={"modal" + "editNotes"} className="modal bottom-sheet">
            <div className=' mx-auto p-0 flex flex-col items-center'>
              <p className='text-md mt-3 mb-1 p-1 underline'> Old Notes : {userProfile.user.notes}</p>
            </div>
            <p className='text-md m-1 p-1 text-center'> Change to </p>
            <div className='row items-center justify-around  max-w-lg mx-auto mt-2'>
              <p className='col s2 text-black items-center justify-around text-md'> New: </p>
              <textarea className='text-sm col s10 browsers-default h-32 border-black border-solid border-2' rows="10" 
